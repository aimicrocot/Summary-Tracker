...
// Состояние скрытия — на чат, как и факты: индексы и память одного чата
// не имеют смысла в другом. Без chatId сеттер молчит, чтобы не записать
// состояние в момент, когда SillyTavern ещё не восстановил чат.
...
function applyVisualHiding() {
    ...
    const active = getIsHidden() && fullContext.length > 0;
...
${numbered}`;
        ...
        // Общая точка обеих веток скана: покрывает и одиночный, и batch, и случай,
        // когда maybeCompressFacts свернул факты в слой. Ветка неразобранного JSON
        // выходит раньше, так что сюда попадают только успешные саммари.
        if (extension_settings[extensionName].autoHide && buildFullContext().length > 0) {
            setIsHidden(true);
            // renderFacts выше отработал до выставления флага — перерисовываем.
            applyVisualHiding();
            updateHideButton();
        }
...
function loadSettings() {
    ...
    // Миграция с версий, где скрытие было одним глобальным флагом. Чату он не
    // принадлежал, поэтому переносим его в текущий чат, если тот уже открыт.
    if (settings.isHidden !== undefined) {
        if (settings.isHidden === true) setIsHidden(true);
